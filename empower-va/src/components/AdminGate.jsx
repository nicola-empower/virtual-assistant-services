...
export default function AdminGate({ children }) {
    ...
    useEffect(() => {
        console.log("AdminGate mounted. Checking session...");
        // Check active session
        try {
            supabase.auth.getSession().then(({ data: { session }, error }) => {
                if (error) console.error("Supabase Session Error:", error);
                console.log("AdminGate initial session:", session);
                setSession(session);
                setLoading(false);
            });
            ...
            const {
                ...
                console.log("AdminGate auth change:", _event, session);
                setSession(session);
            });

            return () => subscription.unsubscribe();
        } catch (err) {
            console.error("AdminGate Supabase Crash:", err);
            setLoading(false);
        }
    }, []);
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            setAuthError("System Error: " + err.message);
            setIsLoggingIn(false);
        }
